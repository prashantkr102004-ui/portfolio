...
export function FeaturedWork() {
  ...
  return <section id="work" className="featured-section" aria-labelledby="work-title"><div className="section-heading"><div><span className="eyebrow mono">01 / Featured Work</span><h2 id="work-title">Serious projects, built around real workflows.</h2></div><p>AI/ML, full-stack systems, data pipelines, and product thinking presented as substantial case studies.</p></div>
    <div className="featured-projects">{featuredProjects.map(project => {
    ...
    <div className="more-work" id="more-projects" aria-labelledby="more-work-title"><div><span className="eyebrow mono">Explore more projects</span><h3 id="more-work-title">Smaller experiments with clear engineering ideas.</h3></div><div className="more-work-list">{moreProjects.map(project => <article className="more-work-item" key={project.id} id={project.id}><span className="mono more-work-number">{project.number}</span><div><span className="eyebrow mono">{project.category}</span><h4><Link href={`/projects/${project.id}`}>{project.name}<Arrow diagonal /></Link></h4><p>{project.description}</p><div className="tech-list mono">{project.stack.map(item => <span key={item}>{item}</span>)}</div></div><a className="repository-link" href={project.repository} target="_blank" rel="noreferrer" aria-label={`${project.name} on GitHub`}>GitHub <Arrow diagonal /></a></article>)}</div></div>
  </section>;
}
